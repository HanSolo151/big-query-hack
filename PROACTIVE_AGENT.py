    ...
    def _setup_gemini(self):
        ...
        # Initialize Gemini
            self.llm = ChatGoogleGenerativeAI(
                model="gemini-1.5-flash",   # or whatever model ProactiveAgent needs
                google_api_key=api_key,
                temperature=0.2,
                max_output_tokens=1024,
            )

            print("✓ Proactive Agent Gemini model configured successfully")

        except Exception as e:
            print(f"✗ Proactive Agent Gemini setup failed: {e}")
            raise
    
    def _setup_embeddings(self):
        """Set up Gemini embeddings model for proactive monitoring"""
        try:
            api_key = os.getenv("GOOGLE_GEMINI_API_KEY")
            if not api_key:
                if not os.path.exists(self.api_key_path):
            ...
            print("✓ Proactive Agent embeddings configured successfully")

        except Exception as e:
            print(f"✗ Proactive Agent embeddings setup failed: {e}")
            raise
